...
const getStats = async (req, res) => {
  try {
    const [totalOrders, totalUsers, totalProducts, totalEvents] =
      await Promise.all([
        prisma.orders.count(),
        prisma.user.count(),
        prisma.item.count(),
...
const createEvent = async (req, res) => {
  try {
    ...
    const event = await prisma.eventType.create({
      data: { name, description, banner_image, is_active: is_active !== false },
    });
...
const updateEvent = async (req, res) => {
  try {
    ...
    const event = await prisma.eventType.update({
      ...
      data: { name, description, banner_image, is_active },
    });
    res.json(event);
  } catch (err) {
...
const updateProduct = async (req, res) => {
  try {
    const { name, description, base_discount, subcategory_id, is_active } =
      req.body;
...
const getAdminCategories = async (req, res) => {
  try {
    const cats = await prisma.category.findMany({
      include: { subcategories: true, upper_category: true },
      orderBy: { id: "asc" },
    });
    res.json(cats);
  } catch (err) {
...
const getAdminUpperCategories = async (req, res) => {
  try {
    const cats = await prisma.upperCategory.findMany({
...
const createUpperCategory = async (req, res) => {
  try {
    ...
    const cat = await prisma.upperCategory.create({
      data: { name, is_active: is_active !== false },
    });
    res.status(201).json(cat);
  } catch (err) {
...
const updateUpperCategory = async (req, res) => {
  try {
    const { name, is_active } = req.body;
    const cat = await prisma.upperCategory.update({
      where: { id: parseInt(req.params.id) },
      data: { name, is_active },
    });
    res.json(cat);
