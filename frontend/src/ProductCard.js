...
function ProductCard({ product }) {
  ...
  const primaryImage = product.variants?.[0]?.images?.find(img => img.is_primary)?.image_url
  ...
  const discountedPrice = lowestPrice && discount
    ...
    <div className="pcard" onClick={() => navigate(`/product/${product.id}`)}>
      <div className="pcard-img-wrap">
        {primaryImage
          ...
          : <div className="pcard-img-placeholder"><span>No Image</span></div>
        }
        {discount > 0 && (
          <span className="pcard-badge">{discount}% OFF</span>
        )}
      </div>
      <div className="pcard-body">
        ...
        <div className="pcard-meta">
          <span className="pcard-variants">{product.variants?.length || 0} variant{product.variants?.length !== 1 ? "s" : ""}</span>
